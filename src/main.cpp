...
// I/O
auto ReadInput(py::array_t<double> input) -> PointVector;
auto WriteOutput(PointVector input) -> py::array_t<double>;

// Algorithm-specific
auto ComputeConcaveHull(py::array_t<double> input, int k, bool iterate = true) -> py::array_t<double>;
...
auto ConcaveHull(PointVector &dataset, size_t k, PointVector &hull, bool iterate) -> bool;
...
auto ComputeConcaveHull(py::array_t<double> input, int k, bool iterate) -> py::array_t<double>
{
	// Read input
	PointVector points = ReadInput(input);
	...
	py::array_t<double> result = WriteOutput(hull);
...
auto ConcaveHull(PointVector &pointList, size_t k, PointVector &hull, bool iterate) -> bool
{
	hull.clear();
...
PYBIND11_MODULE(concave_hull, m)
{
	...
	m.def("compute", &ComputeConcaveHull, R"pbdoc(
		...
		hull : (Mx2) array
		  ...
		  py::arg("input"), py::arg("k"), py::arg("iterate") = true);
}
